...
import { Check, Save, AlertCircle, X, Lock, Clock, ChevronDown, ChevronUp, ArrowLeft } from "lucide-react";
import {
  Box,
  Heading,
  Text,
...
import type { Route } from "./+types/_app.work-sample.$attemptId";
...
export async function loader({ request, params }: Route.LoaderArgs) {
  ...
  const res = await authenticatedFetch(request, `/api/v1/work-sample-attempts/${attemptId}`);
  if (!res.ok) {
...
export default function WorkSampleByAttempt({ loaderData }: Route.ComponentProps) {
  const navigate = useNavigate();

  // Redirect to conversational interview if interview_mode is "conversation"
  useEffect(() => {
    if (loaderData.attempt?.interview_mode === "conversation") {
      navigate(`/app/interview?attempt=${loaderData.attempt.id}`, { replace: true });
    }
  }, [loaderData.attempt?.interview_mode, loaderData.attempt?.id, navigate]);
  ...
  // Handle submit — redirect to dashboard after success
